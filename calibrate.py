...
def main(image_path, fisheye, pattern_size, square_size, threads, json_file=None, debug_dir=None):
    """    
    ...
    test = load_images_from_folder(image_path[0])
    print(test)
    img = cv2.imread(test[0], cv2.IMREAD_GRAYSCALE)
    if img is None:
        print('Failed to read %s to get resolution!' % test[0])
        return
    ...
    image_files = load_images_from_folder(image_path[0])
    if threads <= 1:
    ...
    if debug_dir is not None:
        ...
        image_files = load_images_from_folder("./debug")
        print(image_files)
        for fname in image_files:
            ...
            img = cv2.imread("./debug/"+img_found)
            if img is None:
